...
# init_velocities = init_vel(atom_count,temperature=1.05)
...
# e_out, px_out, py_out, pz_out = calculate_energies(v_out)



cell_lengths = [7.22,7.32,7.44,7.56,7.69]  # Different cell lengths to test
# k_b = 1.0  # Boltzmann constant in reduced units

pbar_2 = tqdm.tqdm(cell_lengths)

# Loop over different cell lengths
for cell_length in pbar_2:
    # Initialize positions, velocities, and forces
    init_positions, atom_count = import_file(filename)
    init_velocities = init_vel(atom_count,temperature=1.)
    init_forces, init_potential, pre= calculate_forces(init_positions,  cell_length)
    ...
    avg_pressure = np.mean(pre)

    #  Save pressure data to a text file
    pressure_filename = f'pressure_cell_length_{cell_length}.txt'
    np.savetxt(pressure_filename, pre)
    # pbar_2.set_postfix_str({f''Pressure':np.around(pre,2)})
    pbar_2.set_postfix_str({f'Pressure': np.around(pre, 2)})
    ...
    plt.plot(time, pre, label=f'Cell Length: {cell_length}')
    plt.xlabel('Time')
    plt.ylabel('Pressure')
    plt.legend()

    # Save the figure as an image file (e.g., PNG)
    fig_filename = f'pressure_vs_time_cell_length_{cell_length}.png'
    plt.savefig(fig_filename)
    
    # Print or store the average pressure and other results
    print(f'Cell Length: {cell_length}, Average Pressure: {avg_pressure}')
